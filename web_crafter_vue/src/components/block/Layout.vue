...
- ✅ 생성기 기본 스타일은 style=""로 넣지 않고 data-wc-style로만 넣음  
...
export const defineBlocks = () => {
  ...
  // ✅ 공통: 생성기 기본 스타일은 data-wc-style로만 넣기 (코드보기에서 완전 제거됨)
  const builderStyleAttr = () => `data-wc-style="${BASE_BOX_STYLE}"`;

  // =========================================================
  // [Area]
  // =========================================================
  Blockly.Blocks['layout_area'] = {
    init() {
      // ✅ 타이틀 단독 1줄
      ...
      this.appendDummyInput('PROPS')
        .appendField('이름:')
        .appendField(new Blockly.FieldTextInput('wc-area'), 'ELEMENT_CLASS')
        .appendField('태그:')
        .appendField(
  ...
  javascriptGenerator.forBlock['layout_area'] = (block) => {
    ...
    const cls = getClass(block, 'wc-area');
    const content = javascriptGenerator.statementToCode(block, 'CONTENT');

    return `<${safeTag} class="${cls}" data-wc-block="container" ${builderStyleAttr()} ${draggableAttrs(block)}>
${content}</${safeTag}>\n`;
  };
  ...
  javascriptGenerator.forBlock['layout_box'] = (block) => {
    ...
    return `<${safeTag} class="${cls}" ${builderStyleAttr()} ${draggableAttrs(block)}>
${content}</${safeTag}>\n`;
  };
  ...
  javascriptGenerator.forBlock['layout_ul'] = (block) => {
    ...
    return `<ul class="${cls}" ${builderStyleAttr()} ${draggableAttrs(block)}>
${items}</ul>\n`;
  };
...
${content}</li>\n`;
  ...
  javascriptGenerator.forBlock['layout_form'] = (block) => {
    ...
    return `<form class="${cls}" ${builderStyleAttr()} ${draggableAttrs(block)}>
${content}</form>\n`;
  };
  ...
  javascriptGenerator.forBlock['layout_form_field'] = (block) => {
    ...
    const extra = adv
      ? safeClassChunk(block.getFieldValue('EXTRA_CLASS') || '')
      : '';
