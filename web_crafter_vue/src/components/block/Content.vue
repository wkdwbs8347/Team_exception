...
export const defineBlocks = () => {
  ...
  const titleWithSep = (block, title) => {
    block.appendDummyInput('TITLE_ROW').appendField(title).appendField('│'); // 여기만 바꾸면 전체 스타일 통일됨 (예: ' —', ' ▸', ' ·')
  };
  ...
  javascriptGenerator.forBlock['content_heading'] = (block) => {
    ...
    return `<${level} class="${safeName}" data-block-id="${block.id}" style="${style}">${text}</${level}>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_button'] = (block) => {
    ...
    return `<button class="${safeName}" data-block-id="${block.id}" style="${style}">${label}</button>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_text'] = (block) => {
    ...
    const text = safeText(block.getFieldValue('TEXT') || '');
    const style = getStyle('');
    return `<span class="${safeName}" data-block-id="${block.id}" style="display:block; ${style}">${text}</span>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_image'] = (block) => {
    ...
    return `<img class="${safeName}" src="${imgSrc}" data-block-id="${block.id}" style="${style}" />\n`;
  };
  ...
  javascriptGenerator.forBlock['content_link'] = (block) => {
    ...
    return `<a class="${safeName}" href="${safeAttr(
      href
    )}"${targetAttr} data-block-id="${block.id}" style="${style}">${text}</a>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_paragraph'] = (block) => {
    ...
    return `<p class="${safeName}" data-block-id="${block.id}" style="${style}">${text}</p>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_label'] = (block) => {
    ...
    return `<label class="${safeName}"${forAttr} data-block-id="${block.id}" style="${style}">${text}</label>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_input'] = (block) => {
    ...
    const style = getStyle('');
    return `<input class="${safeName}" type="${type}"${phAttr}${valueAttr}${reqAttr}${disAttr} data-block-id="${block.id}" style="${style}" />\n`;
  };
  ...
  javascriptGenerator.forBlock['content_textarea'] = (block) => {
    ...
    const style = getStyle('');
    return `<textarea class="${safeName}"${rowsAttr}${phAttr}${reqAttr}${disAttr} data-block-id="${block.id}" style="${style}">${text}</textarea>\n`;
  };
    ...
    init() {
      ...
      const fireOptionCountChange = (block, oldCount, newCount) => {
        try {
          ...
          // Blockly 이벤트 그룹핑(undo/redo 깔끔)
          Blockly.Events.setGroup(true);

          // ✅ mutation 변경 이벤트 발사 (workspace changeListener 트리거용)
          Blockly.Events.fire(
      ...
      // ... content_select init() 내부에서 addBtn/removeBtn 만들던 부분을 아래로 교체
  ...
  javascriptGenerator.forBlock['content_select'] = (block) => {
    ...
    const style = getStyle('');
    return `<select class="${safeName}"${disAttr} data-block-id="${block.id}" style="${style}">
${optionsHtml}</select>\n`;
  };
  ...
  javascriptGenerator.forBlock['content_checkbox'] = (block) => {
    ...
    const style = getStyle('');
    return `<label class="${safeName}" data-block-id="${block.id}" style="${style}">
